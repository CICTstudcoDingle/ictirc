...
export const INDEX_CONFIGS: Record<string, IndexConfig> = {
  ...
  [INDICES.AUTHORS]: {
    ...
    searchableAttributes: ["name", "affiliation", "bio", "researchInterests"],
    attributesForFaceting: [
  ...
  [INDICES.CONFERENCES]: {
    ...
    searchableAttributes: ["name", "description", "location", "organizer"],
    attributesForFaceting: [
      "year",
      "searchable(location)",
...
export async function addObjectsToIndex<T extends { objectID: string }>(
  ...
  try {
    ...
    for (let i = 0; i < objects.length; i += CHUNK_SIZE) {
      ...
      console.log(
        `✅ Added ${chunk.length} objects to ${indexName} (${i + chunk.length}/${objects.length})`,
      );
    }
  } catch (error) {
...
export async function deleteObjectsFromIndex(
  ...
  try {
    ...
    console.error(
      `❌ Failed to delete objects from index ${indexName}:`,
      error,
    );
    throw error;
  }
