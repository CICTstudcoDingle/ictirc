...
async function main() {
  ...
  if (!conference) {
    console.error(
      "❌ No 2026 conference found. Please create it in the admin panel first.",
    );
    process.exit(1);
  }

  console.log(
    `✅ Found conference: "${conference.name}" (id: ${conference.id})`,
  );
  ...
  for (const member of COMMITTEE_2026) {
    ...
    if (existing) {
      console.log(
        `  ⏭  Skipped (already exists): [${member.position}] ${member.name}`,
      );
      skipped++;
      continue;
    }
