    ...
    <section className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-gray-600">
          Incoming students pay a fixed PHP 50 department fee before enrollment.
          Section can stay blank.
        </p>
      </div>

      <div className="grid gap-6 xl:grid-cols-2">
        <form
          action={createDepartmentWithFeeAction}
          className="panel p-5 space-y-3"
        >
          <h2 className="text-lg font-semibold">Department and Fee Setup</h2>
          <p className="text-xs text-gray-500">
            Fee is fixed at PHP 50 for all departments and terms.
          </p>
          <div className="grid gap-3 sm:grid-cols-2">
            <input
              name="code"
              placeholder="Department code (e.g. BSIT)"
              className="input"
              required
            />
            <input
            ...
            <tbody>
              {enrollments.map((enrollment: EnrollmentRecord) => (
                <tr key={enrollment.id} className="border-t border-gray-200">
                  ...
                  <td className="px-4 py-3">
                    PHP {Number(enrollment.departmentFee).toFixed(2)}
                  </td>
                  <td className="px-4 py-3">
                    {enrollment.sectionName || "(blank)"}
                  </td>
                  <td className="px-4 py-3">
