  ...
  try {
    ...
    lines.slice(0, 15).forEach((line, idx) => {
      const fontSize = Math.sqrt(
        line.items[0].transform[0] ** 2 + line.items[0].transform[1] ** 2,
      );
      // Bonus for Uppercase or Bold
      const isUpper =
        line.text === line.text.toUpperCase() && line.text.length > 5;
      const score =
        fontSize * (line.isBold ? 1.2 : 1.0) * (isUpper ? 1.1 : 1.0);
    ...
    if (titleLineIndex !== -1) {
      ...
      while (nextIdx < lines.length) {
        ...
        const nextSize = Math.sqrt(
          nextLine.items[0].transform[0] ** 2 +
            nextLine.items[0].transform[1] ** 2,
        );
      ...
      const abstractLineIndex = lines.findIndex((l) =>
        l.text.match(abstractRegex),
      );
      const keywordsLineIndex = lines.findIndex((l) =>
        l.text.match(keywordsRegex),
      );
      ...
      if (abstractLineIndex !== -1) {
        ...
        const stopIndex =
          keywordsLineIndex !== -1 ? keywordsLineIndex : lines.length;
      ...
      if (emailLineIndices.length > 0) {
        ...
        let affiliation = "";
        const affiliationRelIdx = firstEmailRelIdx - 1;
        ...
        const authorTextLines = metadataLines.slice(
          0,
          Math.max(0, affiliationRelIdx),
        );
        ...
        const rawAuthors = cleanAuthorString
          .split(/,| and /)
          .map((a) => a.trim())
          .filter((a) => a.length > 2);
