...
export function DeleteIssueButton({
  ...
  async function handleClick(e: React.MouseEvent) {
    ...
    if (
      !confirm(
        `Are you sure you want to delete Issue ${issueNumber} of Volume ${volumeNumber}?\n\nThis cannot be undone.`,
      )
    ) {
      return;
    }
    ...
    try {
      ...
      if (result.success) {
        ...
        alert(`Failed to delete issue: ${result.error ?? "Unknown error"}`);
      }
    } catch {
      alert("An unexpected error occurred while deleting the issue.");
    } finally {
      setLoading(false);
    }
  }
