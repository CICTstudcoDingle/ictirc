    ...
    <div className="p-4 md:p-6 space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-gray-600 mt-1">
          Manage Algolia search indices, synchronize data, and monitor search
          performance.
        </p>
      </div>
      ...
      {message && (
        <div
          className={`p-3 md:p-4 rounded-lg flex items-center gap-2 text-sm ${
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
        ...
        <div className="bg-white p-4 md:p-6 rounded-lg border border-gray-200 shadow-sm">
          ...
          <p className="text-xs md:text-sm text-gray-600 mb-3 md:mb-4">
            Synchronize all papers, authors, conferences, and archives to
            Algolia.
          </p>
          <button
        ...
        <div className="bg-white p-4 md:p-6 rounded-lg border border-gray-200 shadow-sm">
          ...
          <button
            onClick={() => {
              if (
                confirm(
                  "Are you sure you want to clear all search indices? This cannot be undone.",
                )
              ) {
                handleAction("clear");
              }
        ...
        <div className="bg-white p-4 md:p-6 rounded-lg border border-gray-200 shadow-sm">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-4">
            {Object.entries(stats).map(([indexName, indexStats]) => (
              <div
                key={indexName}
                className="border border-gray-100 rounded-lg p-3 md:p-4"
              >
                <h4 className="font-medium text-gray-900 mb-2 text-sm font-mono truncate">
                  {indexName}
                </h4>
                {indexStats.error ? (
        ...
        >
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-gray-500">
              Test search results after syncing data
            </p>
          </div>
        ...
        {showPreview && (
          <div className="mt-4 space-y-4">
            ...
            {query && !searchLoading && (
              <p className="text-xs text-gray-500">
                Found{" "}
                <span className="font-medium text-gray-900">{totalHits}</span>{" "}
                results in{" "}
                <span className="font-mono text-[#800000]">
                  {processingTimeMS}ms
                </span>
              </p>
            )}
      ...
      <div className="bg-[#800000]/5 p-4 md:p-6 rounded-lg border border-[#800000]/10">
        <h3 className="text-base md:text-lg font-semibold text-[#800000] mb-2">
          Setup Instructions
        </h3>
        <div className="text-gray-700 space-y-2 text-xs md:text-sm">
          <p>
            1. <strong>Setup Indices:</strong> First-time setup to create
            Algolia indices with proper configuration.
          </p>
          <p>
            2. <strong>Sync Data:</strong> Import all existing data from your
            database to make it searchable.
          </p>
          <p>
            3. <strong>Preview:</strong> Use the Live Search Preview to verify
            data is properly indexed.
          </p>
          <p>
            4. <strong>Monitor:</strong> Use statistics to monitor search
            performance and index health.
          </p>
        </div>
      </div>
    </div>
